...
export function useGoal(id: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Goal>(`/api/goals/${id}`);
      return data;
    },
    enabled: !!id,
  });
...
export function useCreateGoal() {
  ...
  return useMutation({
    mutationFn: async (body: Partial<Goal>) => {
      const { data } = await api.post<Goal>('/api/goals', body);
      return data;
    },
...
export function useUpdateGoal() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...body }: Partial<Goal> & { id: string }) => {
      const { data } = await api.put<Goal>(`/api/goals/${id}`, body);
      return data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: [GOALS_KEY] }),
  });
...
export function useUpdateGoalProgress() {
  ...
  return useMutation({
    mutationFn: async ({ id, progress }: { id: string; progress: number }) => {
      const { data } = await api.patch<Goal>(`/api/goals/${id}/progress`, { progress });
      return data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: [GOALS_KEY] }),
  });
...
export function useMilestones(goalId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<Milestone[]>(`/api/goals/${goalId}/milestones`);
      return data;
    },
    enabled: !!goalId,
  });
...
export function useCreateMilestone() {
  ...
  return useMutation({
    mutationFn: async ({ goalId, title, dueDate }: { goalId: string; title: string; dueDate?: string }) => {
      const { data } = await api.post<Milestone>(`/api/goals/${goalId}/milestones`, { title, dueDate });
      return data;
    },
    onSuccess: (_d, vars) =>
...
export function useToggleMilestone() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data } = await api.patch(`/api/goals/${goalId}/milestones/${milestoneId}`, {
        completed,
      });
      return data;
    },
