...
export function useSessions(params?: { limit?: number; from?: string; to?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<FocusSession[]>('/api/sessions', { params });
      return data;
    },
  });
}
...
export function useStartSession() {
  ...
  return useMutation({
    mutationFn: async (body: {
      ...
      const { data } = await api.post<FocusSession>('/api/sessions', {
        ...body,
...
export function useCompleteSession() {
  ...
  return useMutation({
    mutationFn: async ({ id, notes }: { id: string; notes?: string }) => {
      const { data } = await api.put<FocusSession>(`/api/sessions/${id}`, {
