...
export function useRosterShifts(params?: { from?: string; to?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<RosterShift[]>('/roster/shifts', { params });
      return data;
    },
  });
}
...
export function useCreateShift() {
  ...
  return useMutation({
    mutationFn: async (body: Partial<RosterShift>) => {
      const { data } = await api.post<RosterShift>('/roster/shifts', body);
      return data;
    },
...
export function useUpdateShift() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...body }: Partial<RosterShift> & { id: string }) => {
      const { data } = await api.put<RosterShift>(`/roster/shifts/${id}`, body);
      return data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: [ROSTER_KEY] }),
  });
...
export function useDeleteShift() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.delete(`/roster/shifts/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: [ROSTER_KEY] }),
  });
}
